...
select ok((select relrowsecurity from pg_class where oid='public.visitors'::regclass),'RLS is enabled on visitors');
select ok((select relrowsecurity from pg_class where oid='public.attendance'::regclass),'RLS is enabled on attendance');
select ok(exists(select 1 from pg_constraint where conname='attendance_single_checkin' and conrelid='public.attendance'::regclass),'duplicate attendance constraint exists');
select * from finish();
rollback;
