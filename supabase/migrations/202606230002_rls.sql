...
set search_path = public, pg_temp
as $$
  select public.current_app_role() = 'administrator'::public.app_role;
$$;
...
set search_path = public, pg_temp
as $$
  select public.current_app_role() = 'usher'::public.app_role;
$$;
...
set search_path = public, pg_temp
as $$
  select public.current_app_role() = 'auditor'::public.app_role;
$$;

create or replace function public.service_is_authorized(p_service_id uuid)
returns boolean
language sql
stable
security definer
    ...
    where s.id = p_service_id
      ...
      and (
        ...
        or (
          public.is_usher()
          and (
            settings.require_service_assignment = false
            or exists (
              select 1
              from public.service_assignments assignment
              where assignment.service_id = s.id
...
to authenticated
using (id = auth.uid() and public.current_profile_is_valid());

drop policy if exists profiles_admin_select_org on public.user_profiles;
create policy profiles_admin_select_org
on public.user_profiles for select
to authenticated
using (public.is_admin() and public.has_aal2() and organization_id = public.current_org_id());
...
create policy retention_admin_select
on public.retention_actions for select
to authenticated
using (public.is_admin() and public.has_aal2() and organization_id = public.current_org_id());
...
-- The visitors table is intentionally not granted directly to ordinary authenticated users.
-- Visitor searches and writes are exposed only through validated functions below.
