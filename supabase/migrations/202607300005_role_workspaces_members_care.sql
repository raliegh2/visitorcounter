...
as $$
  select exists (
    ...
    where p.id = public.current_actor_id()
      ...
      and coalesce(to_timestamp(nullif(auth.jwt() ->> 'iat', '')::bigint), now()) >= p.auth_not_before
  );
$$;
...
declare
  ...
  v_requested text := lower(coalesce(new.raw_user_meta_data ->> 'requested_role', 'usher'));
...
begin
  ...
  if v_org_id is not null and exists (select 1 from public.organizations where id = v_org_id) then
    v_invited := true;
  else
  ...
  if v_invited and new.raw_app_meta_data ->> 'role' in ('administrator', 'usher', 'pastor', 'auditor') then
    ...
    v_status := 'approved';
    if v_role = 'pastor' then v_requested := 'pastor';
    elsif v_role = 'administrator' then v_requested := 'administrator';
    else v_requested := 'usher'; end if;
  elsif v_requested in ('pastor', 'administrator') then
  ...
  on conflict (id) do update set
    ...
    requested_role = case when public.user_profiles.role = 'administrator' then public.user_profiles.requested_role else excluded.requested_role end,
    role_status = case when public.user_profiles.role = 'administrator' then public.user_profiles.role_status else excluded.role_status end,
    role = case when public.user_profiles.role = 'administrator' then public.user_profiles.role else excluded.role end,
    active = true,
    updated_at = now();
  ...
  if v_requested = 'pastor' and v_status = 'pending' then
    ...
    values (
      new.id,
      v_org_id,
      left(coalesce(nullif(btrim(new.raw_user_meta_data ->> 'church_name'), ''), 'Not provided'), 160),
      left(coalesce(nullif(btrim(new.raw_user_meta_data ->> 'pastor_name'), ''), 'Not provided'), 120),
      left(coalesce(nullif(btrim(new.raw_user_meta_data ->> 'district'), ''), 'Not provided'), 160),
      left(coalesce(nullif(btrim(new.raw_user_meta_data ->> 'denomination'), ''), 'Not provided'), 160),
...
as $$
  select
    (select count(*) from public.visitors v where v.organization_id = public.current_org_id() and v.active and v.anonymized_at is null),
    case when public.is_pastor_or_admin() then (select count(*) from public.members m where m.organization_id = public.current_org_id()) else 0 end,
    (select count(*) from public.care_notes n where n.organization_id = public.current_org_id() and n.status <> 'resolved' and (public.is_pastor_or_admin() or (n.visitor_id is not null and n.visibility = 'assigned_team'))),
    (select count(*) from public.visit_records r where r.organization_id = public.current_org_id() and r.outcome = 'completed' and (public.is_pastor_or_admin() or r.visitor_id is not null)),
    case when public.is_admin() then (select count(*) from public.user_profiles p where p.organization_id = public.current_org_id() and p.role_status = 'pending') else 0 end
  where public.current_profile_is_valid();
$$;

create or replace function public.search_member_records(p_query text default '')
returns table(
  id uuid, first_name text, last_name text, email text, phone text, address text,
  membership_status text, last_contact_at timestamptz, last_visited_at timestamptz,
  visit_count bigint, has_been_visited boolean
)
language plpgsql
stable
security definer
...
begin
  ...
  return query
  select m.id, m.first_name, m.last_name, m.email, m.phone, m.address, m.membership_status,
    m.last_contact_at,
    (select max(vr.visited_at) from public.visit_records vr where vr.member_id = m.id) as last_visited_at,
    (select count(*) from public.visit_records vr where vr.member_id = m.id) as visit_count,
    exists(select 1 from public.visit_records vr where vr.member_id = m.id and vr.outcome = 'completed') as has_been_visited
  from public.members m
  where m.organization_id = public.current_org_id()
    and (v_query = '' or concat_ws(' ', m.first_name, m.last_name) ilike '%' || replace(replace(v_query, '%', '\\%'), '_', '\\_') || '%' escape '\\'
...
create or replace function public.create_member_record(
  p_first_name text, p_last_name text, p_email text, p_phone text, p_address text,
  p_membership_status text, p_last_contact_at text
)
returns uuid
language plpgsql
security definer
...
begin
  ...
  insert into public.members (organization_id, first_name, last_name, email, phone, address, membership_status, last_contact_at, created_by)
  values (public.current_org_id(), btrim(p_first_name), btrim(p_last_name), nullif(btrim(coalesce(p_email, '')), ''), nullif(btrim(coalesce(p_phone, '')), ''), nullif(btrim(coalesce(p_address, '')), ''), v_status, case when nullif(btrim(coalesce(p_last_contact_at, '')), '') is null then null else p_last_contact_at::timestamptz end, public.current_actor_id())
  returning id into v_id;
  perform public._write_audit('MEMBER_CREATED', 'member', v_id, 'success', jsonb_build_object('membership_status', v_status));
  return v_id;
end;
$$;
...
set search_path = public, pg_temp
as $$
declare v_row jsonb; v_count integer := 0; v_batch uuid := gen_random_uuid(); v_id uuid; v_status text;
begin
  if not public.is_pastor_or_admin() then raise exception 'authorization denied'; end if;
  ...
  for v_row in select value from jsonb_array_elements(p_rows) loop
    v_status := lower(coalesce(v_row ->> 'membership_status', 'active'));
    if v_status not in ('active', 'inactive', 'prospective') then v_status := 'active'; end if;
    ...
    insert into public.members (organization_id, first_name, last_name, email, phone, address, membership_status, last_contact_at, imported_at, import_batch_id, created_by)
    values (public.current_org_id(), btrim(v_row ->> 'first_name'), btrim(v_row ->> 'last_name'), nullif(btrim(coalesce(v_row ->> 'email', '')), ''), nullif(btrim(coalesce(v_row ->> 'phone', '')), ''), nullif(btrim(coalesce(v_row ->> 'address', '')), ''), v_status, case when nullif(btrim(coalesce(v_row ->> 'last_contact_at', '')), '') is null then null else (v_row ->> 'last_contact_at')::timestamptz end, now(), v_batch, public.current_actor_id())
    returning id into v_id;
    v_count := v_count + 1;
  end loop;
  perform public._write_audit('MEMBERS_IMPORTED', 'member_import', v_batch, 'success', jsonb_build_object('row_count', v_count));
  return v_count;
end;
$$;
...
begin
  ...
  return query
    select 'visitor'::text, v.id, coalesce(nullif(v.preferred_name, ''), v.full_name), concat('First visit ', v.first_visit_date::text), v.optional_contact,
      (select max(r.visited_at) from public.visit_records r where r.visitor_id = v.id),
      (select count(*) from public.visit_records r where r.visitor_id = v.id)
    from public.visitors v
    where v.organization_id = public.current_org_id() and v.active and v.anonymized_at is null
      and (v_query = '' or v.id = v_uuid or v.full_name ilike '%' || replace(replace(v_query, '%', '\\%'), '_', '\\_') || '%' escape '\\' or coalesce(v.preferred_name, '') ilike '%' || replace(replace(v_query, '%', '\\%'), '_', '\\_') || '%' escape '\\')
  union all
    select 'member'::text, m.id, concat_ws(' ', m.first_name, m.last_name), m.membership_status, coalesce(m.email, m.phone),
      (select max(r.visited_at) from public.visit_records r where r.member_id = m.id),
      (select count(*) from public.visit_records r where r.member_id = m.id)
    from public.members m
    where p_include_members and public.is_pastor_or_admin() and m.organization_id = public.current_org_id()
      and (v_query = '' or m.id = v_uuid or concat_ws(' ', m.first_name, m.last_name) ilike '%' || replace(replace(v_query, '%', '\\%'), '_', '\\_') || '%' escape '\\')
...
begin
  ...
  return query select n.id, n.note_text, n.status, n.visibility, n.created_at, p.display_name
  from public.care_notes n join public.user_profiles p on p.id = n.created_by
  where n.organization_id = public.current_org_id()
    and ((p_person_type = 'visitor' and n.visitor_id = p_person_id) or (p_person_type = 'member' and n.member_id = p_person_id))
    and (public.is_pastor_or_admin() or (n.visitor_id is not null and n.visibility = 'assigned_team'))
...
begin
  if public.current_app_role() not in ('administrator', 'pastor', 'usher') then raise exception 'authorization denied'; end if;
  if p_person_type = 'member' and not public.is_pastor_or_admin() then raise exception 'authorization denied'; end if;
  return query select r.id, r.visited_at, r.outcome, r.summary, p.display_name
  from public.visit_records r join public.user_profiles p on p.id = r.visited_by
  where r.organization_id = public.current_org_id() and ((p_person_type = 'visitor' and r.visitor_id = p_person_id) or (p_person_type = 'member' and r.member_id = p_person_id))
  order by r.visited_at desc limit 200;
end;
$$;
...
begin
  if public.current_app_role() not in ('administrator', 'pastor', 'usher') then raise exception 'authorization denied'; end if;
  if p_person_type = 'member' and not public.is_pastor_or_admin() then raise exception 'authorization denied'; end if;
  if public.current_app_role() = 'usher' then v_visibility := 'assigned_team'; end if;
  if v_visibility not in ('assigned_team', 'pastoral_team', 'admin_only') then raise exception 'invalid visibility'; end if;
  if v_visibility = 'admin_only' and not public.is_admin() then raise exception 'authorization denied'; end if;
  if char_length(btrim(coalesce(p_note_text, ''))) not between 2 and 5000 then raise exception 'invalid note'; end if;
  insert into public.care_notes (organization_id, visitor_id, member_id, note_text, visibility, created_by)
  values (public.current_org_id(), case when p_person_type = 'visitor' then p_person_id else null end, case when p_person_type = 'member' then p_person_id else null end, btrim(p_note_text), v_visibility, public.current_actor_id())
  returning id into v_id;
  perform public._write_audit('CARE_NOTE_CREATED', 'care_note', v_id, 'success', jsonb_build_object('person_type', p_person_type, 'visibility', v_visibility));
  return v_id;
end;
$$;
...
begin
  if public.current_app_role() not in ('administrator', 'pastor', 'usher') then raise exception 'authorization denied'; end if;
  if p_person_type = 'member' and not public.is_pastor_or_admin() then raise exception 'authorization denied'; end if;
  if v_outcome not in ('planned', 'attempted', 'completed', 'follow_up_needed') then raise exception 'invalid visit outcome'; end if;
  if char_length(coalesce(p_summary, '')) > 2000 then raise exception 'visit summary is too long'; end if;
  insert into public.visit_records (organization_id, visitor_id, member_id, visited_by, outcome, summary)
  values (public.current_org_id(), case when p_person_type = 'visitor' then p_person_id else null end, case when p_person_type = 'member' then p_person_id else null end, public.current_actor_id(), v_outcome, nullif(btrim(coalesce(p_summary, '')), ''))
  returning id into v_id;
  perform public._write_audit('MINISTRY_VISIT_RECORDED', 'visit_record', v_id, 'success', jsonb_build_object('person_type', p_person_type, 'outcome', v_outcome));
  return v_id;
end;
$$;
...
begin
  ...
  return query select p.id, p.display_name, u.email::text, p.requested_role, p.role_status, a.church_name, a.pastor_name, a.district, a.denomination, a.church_phone, a.submitted_at
  from public.user_profiles p join auth.users u on u.id = p.id left join public.pastor_applications a on a.profile_id = p.id
  where p.organization_id = public.current_org_id() and p.role_status = 'pending'
  order by coalesce(a.submitted_at, p.created_at);
end;
$$;
...
begin
  if not public.is_admin() then raise exception 'authorization denied'; end if;
  if p_user_id = public.current_actor_id() then raise exception 'self approval is not permitted'; end if;
  select requested_role into v_requested from public.user_profiles where id = p_user_id and organization_id = v_org and role_status = 'pending' for update;
  if not found then raise exception 'role request unavailable'; end if;
  if v_requested not in ('usher', 'pastor', 'administrator') then raise exception 'invalid requested role'; end if;
...
begin
  if not public.is_admin() then raise exception 'authorization denied'; end if;
  if p_user_id = public.current_actor_id() then raise exception 'self role changes are not permitted'; end if;
