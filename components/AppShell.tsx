    ...
    <div className="application-shell">
      <aside className="sidebar">
        <div className="brand">
          ...
          <div><strong>Visitor Care</strong><span>Church attendance</span></div>
        </div>
        <nav aria-label="Main navigation">
          {navigation.filter((item) => item.roles.includes(profile.role)).map((item) => (
            <Link key={item.href} href={item.href} className="nav-link">{item.label}</Link>
          ))}
        </nav>
        <div className="sidebar-account">
          <span>Signed in as</span><strong>{profile.display_name}</strong>
          <small>{profile.role.replace("_", " ")}</small>
          <form action={logoutAction}>
            <button className="button button-secondary button-full" type="submit">Sign out</button>
          </form>
        </div>
      </aside>
