...
export const signupSchema = z.object({
  ...
  email: z.email().max(254),
  requestedRole: z.enum(["usher", "pastor", "administrator"]),
  churchName: z.string().trim().max(160),
  pastorName: z.string().trim().max(120),
  district: z.string().trim().max(160),
  denomination: z.string().trim().max(160),
  churchPhone: z.string().trim().max(40)
}).superRefine((value, context) => {
  if (value.requestedRole === "pastor") {
    for (const [field, text] of Object.entries({
      churchName: value.churchName,
      pastorName: value.pastorName,
      district: value.district,
      denomination: value.denomination,
      churchPhone: value.churchPhone
    })) {
      if (text.length < 2) context.addIssue({ code: "custom", path: [field], message: "This field is required for pastor access." });
    }
  }
});
...
export const visitorRegistrationSchema = z.object({
  ...
  if (value.optionalContact && !value.contactConsent) {
    context.addIssue({ code: "custom", path: ["contactConsent"], message: "Consent is required before storing optional contact information." });
  }
});
...
export const userRoleSchema = z.object({ userId: z.uuid(), role: z.enum(["administrator", "usher", "pastor", "auditor"]) });
...
export const roleReviewSchema = z.object({ userId: z.uuid(), decision: z.enum(["approve", "reject"]), notes: z.string().trim().max(1000) });
...
export const memberSchema = z.object({
  ...
  membershipStatus: z.enum(["active", "inactive", "prospective"]),
  lastContactAt: z.union([z.literal(""), z.iso.datetime({ offset: true }), z.iso.date()])
});

export const memberImportRowSchema = memberSchema;
export const memberImportSchema = z.array(memberImportRowSchema).min(1).max(500);

export const careNoteSchema = z.object({
  personType: z.enum(["visitor", "member"]),
  personId: z.uuid(),
  noteText: z.string().trim().min(2).max(5000),
  visibility: z.enum(["assigned_team", "pastoral_team", "admin_only"])
});

export const ministryVisitSchema = z.object({
  personType: z.enum(["visitor", "member"]),
  personId: z.uuid(),
  outcome: z.enum(["planned", "attempted", "completed", "follow_up_needed"]),
